<template>
    <div class="position-relative" :class="{'loading-holder': isLoading}">
        ...
        @ready="bindSlideClick">
            ...
            <div class="swiper-button-prev" slot="button-prev"><font-awesome-icon icon="angle-left"/></div>
            <div class="swiper-button-next" slot="button-next"><font-awesome-icon icon="angle-right"/></div>
        </swiper>
    </div>
</template>
      ...
      vm.$http.get(api)
        .then((res) => {
          ...
          if (vm.filter === undefined) {
            ...
            vm.allData = products.filter((item) => (item.category === vm.filter.category && item.title !== vm.filter.title && item.is_enabled));
          }
          vm.isLoading = false;
        });
    },
